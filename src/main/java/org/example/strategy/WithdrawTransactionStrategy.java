/**
 * Класс WithdrawTransactionStrategy представляет стратегию транзакции для операции пополнения счета.
...
public class WithdrawTransactionStrategy extends Transaction implements TransactionStrategy{

    /**
     * Конструктор класса WithdrawTransactionStrategy.
     * Создает новый объект WithdrawTransactionStrategy с заданными параметрами.
     * @param account объект типа Счет, с которого будет снята сумма транзакции
     * @param amount сумма транзакции
     * @param currency валюта транзакции
     */
    public WithdrawTransactionStrategy(Account account, double amount, String currency) {
    ...
    public boolean execute() {
        ...
        try {
            if(account1.getBalance() < amount) throw new RuntimeException();
    ...
    public String print() {
        return String.format("-".repeat(44) + "\n" + "| " + " ".repeat(13) + "Банковский чек" +  " ".repeat(13) + " |\n" + "| %-4s %35s |\n", "Чек:",  this.getId()) +
                ...
                String.format("| %-6s %29.2f %3s |\n",  "Сумма:", (-1*this.getAmount()), this.getCurrency()) +
                "|"+ "-".repeat(42) +"|\n";
    }
}
